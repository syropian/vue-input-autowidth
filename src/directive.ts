...
const checkWidth = (
  ...
  let newWidth = mirror.scrollWidth + 2;
...
const mergeDefaultWithOptions = (options: InputAutoWidthOptions) =>
...
export default {
  ...
  mounted: function (
    ...
    let options: Complete<InputAutoWidthOptions> = mergeDefaultWithOptions(
      binding.value
    );
    const hasVModel = !!vnode?.props?.hasOwnProperty("@onUpdate:modelValue");
  ...
  unmounted: function (el: InputWithMirror, binding: DirectiveBinding) {
    ...
    el.removeEventListener(
      "input",
      checkWidth.bind(null, el, mergeDefaultWithOptions(binding.value))
    );
  },
} as ObjectDirective;
